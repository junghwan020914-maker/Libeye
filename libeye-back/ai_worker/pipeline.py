...
def process_scan_session(session_id: str):
    ...
    try:
        ...
        # 💡 [추가] 중복매칭(DUPLICATE)은 2권 이상이 한 세트로 발생하므로, 세트 크기의 몫(N//2)만 실패로 집계
        #   (예: 2권 중복 → 1, 3권 중복 → 1, 4권 중복 → 2)
        duplicate_count = sum(matched_ids.count(bid) // 2 for bid in duplicate_book_ids)
